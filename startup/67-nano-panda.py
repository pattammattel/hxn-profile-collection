...
def timescan_pd(detectors, num, dwell, *,
                      panda_flyer, dead_time = 0,
    ...
    # Set up the merlin
    for det_name in ("merlin2", "eiger2", "eiger_mobile", "eiger3"):
    ...
    for d in panda_flyer.detectors:
        if d.name == 'eiger2' or d.name == 'eiger3' or  d.name == 'eiger_mobile':
    ...
    def plan():
        ...
        yield from bps.sleep(np.abs(num_total*dwell + delta/2))
...
def scan_and_fly_2dpd(detectors, xcenter, xrange, xnum, ystart, ystop, ynum, dwell, *,
    ...
    export_scan_header(hxntools.scans.get_last_scan_id()+1,xmotor,xrange,xnum,ymotor,(ystop-ystart),ynum,0,\
    ...
    def plan():
        ...
        # ## This is assuming one and only one of the below detectors is used, needs improved logic...
        if "merlin2" in dets_by_name:
            # print(f"Configuring 'merlin2' ...")
            dpc = dets_by_name["merlin2"]
            yield from abs_set(dpc.cam.num_images, num_total, wait=True)
        ...
        counter = dpc.hdf5.num_captured.get()
        dot_count = 1
        print("Stages scan completed")
        while counter<num_total:
            print(f"\rDetector {dpc.name} processing not finished, waiting{'.'*dot_count}        ",end='')
            dot_count = (dot_count%3) + 1
            yield from bps.sleep(0.3)
            counter_new = dpc.hdf5.num_captured.get()
...
def pt_timescanpd(dets, num, exposure_time, position_supersample= 10, **kwargs):
    """
    Relative scan
    """
    do_scan = True
